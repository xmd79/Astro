...
# Enhanced Fear and Greed Index with Current Status
def calculate_fear_greed_index(positions, cycles, aspects):
    ...
    # Define ranges and descriptions
    ranges = [
    ...
    trend_direction = "Upward" if lilith_velocity > 0 else "Downward"  # Based on Lilith's motion
    trans_list = transitions[trend_direction]
    current_status = "Stable at " + description
    for i, (label, thresh, deg) in enumerate(trans_list):
        if fear_greed_index < thresh:
            prev_label = trans_list[i-1][0].split(" to ")[0] if i > 0 else description
            current_status = f"Transitioning from {prev_label} to {label.split(' to ')[1]} (Index: {fear_greed_index:.2f}, Threshold: {thresh:.1f})"
            break
        elif fear_greed_index == thresh:
            current_status = f"At transition point: {label} (Index: {fear_greed_index:.2f})"
            break
    else:
        current_status = f"Stable at {description} (Index: {fear_greed_index:.2f})"

    return fear_greed_index, description, lilith_aspects, range_desc, transition_desc, current_status
...
def calculate_planetary_positions():
    try:
        ...
        fear_greed_index, description, lilith_aspects, range_desc, transition_desc, current_status = calculate_fear_greed_index(positions, cycles, aspects)
