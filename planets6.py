...
def get_planet_data(current_time, planet_name):
    ...
    # Initialize the ephem planet object
    ...
    # Calculate the planet's distance from earth in kilometers
    planet_distance_km = planet.earth_distance * ephem.meters_per_au / 1000

    # Calculate the planet's angular diameter in degrees
    planet_angular_diameter = math.degrees(planet.size / planet_distance_km)

    # Calculate the planet's speed in kilometers per hour
    planet_speed_km_hr = planet_distance_km / (1 / 24)

    # Calculate the planet's energy level (dummy value for demonstration)
    planet_energy = planet.elong

    # Calculate the current planet sign
...
# Function to get all planetary data
...
for planet, pos in planet_positions.items():
    print(f"{planet} - RA: {pos['RA']}, DEC: {pos['DEC']}")
print(f"Sun - RA: {sun_position['RA']}, DEC: {sun_position['DEC']}")
...
for planet_name in planets:
    planet_data = get_planet_data(current_time, planet_name)
    print(f"{planet_data['name']} - RA: {planet_data['ra']}, DEC: {planet_data['dec']}, "
          f"Distance (km): {planet_data['distance_km']:.2f}, Speed (km/hr): {planet_data['speed_km_hr']:.2f}, "
          f"Energy: {planet_data['energy']:.2f}, Sign: {planet_data['sign']}")
