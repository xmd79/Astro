...
# Aspects calculation (including 30°, 60°, 120° from the diagram)
...
iching_hexagrams = [
    (1, "Ch'ien", "The Creative", "Strength, initiative"),
    (2, "K'un", "The Receptive", "Devotion, adaptability"),
    (3, "Chun", "Difficulty at the Beginning", "Perseverance through obstacles"),
    (4, "Meng", "Youthful Folly", "Learning, openness"),
    (5, "Hsu", "Waiting", "Patience, preparation"),
    (6, "Sung", "Conflict", "Resolution, clarity"),
    (64, "Wei Chi", "Before Completion", "Transition, caution")
]

# I Ching generation based on astrological data
def generate_iching_hexagram(phase_angle, asc_deg):
    seed = int(phase_angle + asc_deg) % 64 + 1
    for hex_num, name, title, meaning in iching_hexagrams:
        if hex_num == seed:
            return hex_num, name, title, meaning
    return random.choice(iching_hexagrams)
...
# Calculate trigonometric relationships between houses and planets
def calculate_trigonometric_relationships(positions, house_cusps):
    trig_relationships = []
    trig_angles = {
        "Semisextile": 30,
        "Sextile": 60,
    ...
    for house, cusp in house_cusps.items():
        for planet, lon in positions.items():
            angle = min((cusp - lon) % 360, (lon - cusp) % 360)
            for trig_type, target in trig_angles.items():
                if abs(angle - target) <= 4:
                    trig_relationships.append((house, planet, trig_type, round(angle, 2)))
    return trig_relationships

# Draw trigonometric connections on the plot
def draw_trigonometric_connections(ax, positions, house_cusps, trig_relationships):
    trig_styles = {
        "Semisextile": ("cyan", 1, "--"),
        "Sextile": ("lime", 1.2, "-"),
        "Trine": ("magenta", 1.5, "-")
    }
    for house, planet, trig_type, _ in trig_relationships:
        cusp = house_cusps[house]
        lon = positions[planet]
        angle1 = math.radians(360 - cusp + 90)
        angle2 = math.radians(360 - lon + 90)
        x1, y1 = 0.95 * math.cos(angle1), 0.95 * math.sin(angle1)
        x2, y2 = 0.9 * math.cos(angle2), 0.9 * math.sin(angle2)
        color, linewidth, linestyle = trig_styles.get(trig_type, ("gray", 1, "-"))
        ax.plot([x1, x2], [y1, y2], color=color, linewidth=linewidth, linestyle=linestyle, alpha=0.6)

# Astrological wheel plot with all data
def plot_astrological_wheel(positions, moon_phase, local_time_str, aspects, house_cusps, trig_relationships):
    ...
    for house, cusp in house_cusps.items():
        ...
        ax.text(x2 * 1.1, y2 * 1.1, house, fontsize=8, ha='center', va='center', rotation=-(360 - cusp + 90))
    
    # Draw planets
    for planet, lon in positions.items():
        angle = math.radians(360 - lon + 90)
        x, y = 0.9 * math.cos(angle), 0.9 * math.sin(angle)
        ax.plot(x, y, 'o', label=planet, markersize=8)
        ax.text(x * 1.05, y * 1.05, planet, fontsize=8, ha='center', va='center', rotation=-(360 - lon + 90))
    ...
    # Draw trigonometric connections
    draw_trigonometric_connections(ax, positions, house_cusps, trig_relationships)
    
    # Add central text with moon phase and additional data
    ax.text(0, 0, f"THE 12 SIGNS\nMoon Phase: {moon_phase}\nDate: {local_time_str}", ha='center', va='center', fontsize=12, color='purple')
    
    # Add legend
    ax.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=8)
    plt.title(f"Astrological Wheel for {local_time_str}", pad=20)
    
    # Save and show plot
    plt.savefig('astrological_wheel.png')
    plt.show()
...
def astrological_clock():
    ...
    print("\nI Ching Divination:")
    print("-" * 40)
    hex_num, hex_name, hex_title, hex_meaning = generate_iching_hexagram(phase_angle, asc_deg)
    ...
    print("\nTrigonometric Relationships (30°, 60°, 120°):")
    print("-" * 40)
    trig_relationships = calculate_trigonometric_relationships(positions, house_cusps)
    for house, planet, trig_type, angle in trig_relationships:
        print(f"{house} {trig_type} {planet} ({angle}°)")

    # Planetary Frequencies and Elemental Resonance
    print("\nPlanetary Frequencies and Elemental Resonance:")
    print("-" * 40)
    for house, cusp in house_cusps.items():
        sign, element, *_ = get_zodiac_sign(math.radians(cusp))
        planet_in_house = None
        for planet, lon in positions.items():
            if cusp <= lon < (cusp + 30) % 360:
                planet_in_house = planet
                break
        if planet_in_house:
            freq = planetary_frequencies.get(planet_in_house, 0)
            print(f"{house} ({sign}, {element}): {planet_in_house} at {freq:.2f} Hz")
        else:
            print(f"{house} ({sign}, {element}): No planet in this house")

    # Plot
    local_time_str = input_time.strftime("%B %d, %Y, %H:%M %Z")
    print("\nGenerating Astrological Wheel Plot...")
    plot_astrological_wheel(positions, moon_phase, local_time_str, aspects, house_cusps, trig_relationships)
