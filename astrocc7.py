...
def calculate_planetary_cycles(observer, planets, positions, local_time, timezone):
    ...
    for planet in planets:
        ...
        start_tuple = [int(x) for x in start_date.tuple()[:6]]  # Cast floats to integers
        end_tuple = [int(x) for x in end_date.tuple()[:6]]      # Cast floats to integers
...
# Fear and Greed Index for a single planet
...
def calculate_fear_greed_index(positions, cycles, aspects):
    ...
    # Define past and next with reversals
...
def generate_cosmic_reflection(local_time, asc_sign, mc_sign, aspects, patterns, positions, cycles, analysis, 
                              fear_greed_index, description, lilith_aspects, sacred_geo, overall_fgi, overall_desc, sidereal_fgi, sidereal_desc):
